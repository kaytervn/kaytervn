...
const Contact = () => {
  ...
  const { contact: apiList, loading: loadingList } = useApi();
  const { tag } = useApi();
  const {
    data,
    query,
  ...
  const columns = [
    ...
    renderActionButton({
      role: [PAGE_CONFIG.UPDATE_CONTACT.role, PAGE_CONFIG.DELETE_CONTACT.role],
      renderChildren: (item: any) => {
        return (
          <>
            <ActionEditButton
              role={PAGE_CONFIG.UPDATE_CONTACT.role}
              onClick={() => onUpdateButtonClick(item.id)}
            />
            <ActionDeleteButton
              role={PAGE_CONFIG.DELETE_CONTACT.role}
  ...
  const onDeleteButtonClick = (id: any) => {
    showModal(
      configDeleteDialog({
        label: PAGE_CONFIG.DELETE_CONTACT.label,
        deleteApi: () => contact.del(id),
  ...
  const onCreateButtonClick = () => {
    navigate(PAGE_CONFIG.CREATE_CONTACT.path, { state: { query } });
  };

  const onUpdateButtonClick = (id: any) => {
    navigate(`/contact/update/${id}`, { state: { query } });
  };
      ...
      renderContent={
        <>
          ...
          <ToolBar
            ...
            actionButtons={
              <CreateButton
                role={PAGE_CONFIG.CREATE_CONTACT.role}
