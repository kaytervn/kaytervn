...
const IdNumber = () => {
  ...
  const { idNumber: apiList, loading: loadingList } = useApi();
  const { tag } = useApi();
  const {
    data,
    query,
  ...
  const columns = [
    ...
    renderActionButton({
      role: [
        PAGE_CONFIG.UPDATE_ID_NUMBER.role,
        PAGE_CONFIG.DELETE_ID_NUMBER.role,
      ],
      renderChildren: (item: any) => {
        return (
          <>
            <ActionEditButton
              role={PAGE_CONFIG.UPDATE_ID_NUMBER.role}
              onClick={() => onUpdateButtonClick(item.id)}
            />
            <ActionDeleteButton
              role={PAGE_CONFIG.DELETE_ID_NUMBER.role}
  ...
  const onDeleteButtonClick = (id: any) => {
    showModal(
      configDeleteDialog({
        label: PAGE_CONFIG.DELETE_ID_NUMBER.label,
        deleteApi: () => idNumber.del(id),
  ...
  const onCreateButtonClick = () => {
    navigate(PAGE_CONFIG.CREATE_ID_NUMBER.path, { state: { query } });
  };

  const onUpdateButtonClick = (id: any) => {
    navigate(`/id-number/update/${id}`, { state: { query } });
  };
      ...
      renderContent={
        <>
          ...
          <ToolBar
            ...
            actionButtons={
              <CreateButton
                role={PAGE_CONFIG.CREATE_ID_NUMBER.role}
