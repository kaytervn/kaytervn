...
const CreateKeyInformation = () => {
  ...
  const handleSubmit = async () => {
    if (isValidForm()) {
      ...
      const res = await keyInformation.create(submitForm);
      if (res.result) {
  ...
  return (
    <Sidebar
      breadcrumbs={[
        ...
        { label: PAGE_CONFIG.CREATE_KEY_INFORMATION.label },
      ]}
      ...
      renderContent={
        <>
          ...
          <FormCard
            title={PAGE_CONFIG.CREATE_KEY_INFORMATION.label}
            children={
              <div className="flex flex-col space-y-4">
                <div className="flex flex-row space-x-2">
                  <StaticSelectField
...
export default CreateKeyInformation;
