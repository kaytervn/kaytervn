...
import CreateKeyInformationTag from "./CreateKeyInformationTag";
import UpdateKeyInformationTag from "./UpdateKeyInformationTag";

const initQuery = { name: "", kind: "" };

const KeyInformationTag = () => {
  ...
  const columns = [
    ...
    renderActionButton({
      role: [
        PAGE_CONFIG.DELETE_KEY_INFORMATION_TAG.role,
        PAGE_CONFIG.UPDATE_KEY_INFORMATION_TAG.role,
      ],
      renderChildren: (item: any) => (
        <>
          <ActionEditButton
            role={PAGE_CONFIG.UPDATE_KEY_INFORMATION_TAG.role}
            onClick={() => onUpdateButtonClick(item.id)}
          />
          <ActionDeleteButton
            role={PAGE_CONFIG.DELETE_KEY_INFORMATION_TAG.role}
  ...
  const onDeleteButtonClick = (id: any) => {
    showDeleteDialog(
      configDeleteDialog({
        label: PAGE_CONFIG.DELETE_KEY_INFORMATION_TAG.label,
  ...
  const onCreateButtonClick = () => {
    showCreateForm(
      configModalForm({
        label: PAGE_CONFIG.CREATE_KEY_INFORMATION_TAG.label,
        fetchApi: tag.create,
  ...
  const onUpdateButtonClick = (id: any) => {
    showUpdateForm(
      configModalForm({
        label: PAGE_CONFIG.UPDATE_KEY_INFORMATION_TAG.label,
        fetchApi: tag.update,
        refreshData: handleRefreshData,
        hideModal: hideUpdateForm,
        setToast,
  ...
  return (
    <Sidebar
      breadcrumbs={[
        {
          label: PAGE_CONFIG.KEY_INFORMATION_TAG.label,
        },
      ]}
      activeItem={PAGE_CONFIG.KEY_INFORMATION_TAG.name}
      renderContent={
        <>
          ...
          <ToolBar
            ...
            actionButtons={
              <CreateButton
                role={PAGE_CONFIG.CREATE_KEY_INFORMATION_TAG.role}
          ...
          />
          <CreateKeyInformationTag
            isVisible={createFormVisible}
            formConfig={createFormConfig}
          />
          <UpdateKeyInformationTag
...
export default KeyInformationTag;
