...
import CreateServiceTag from "./CreateServiceTag";
import UpdateServiceTag from "./UpdateServiceTag";
...
const ServiceTag = () => {
  ...
  const columns = [
    ...
    renderActionButton({
      role: [
        PAGE_CONFIG.DELETE_SERVICE_TAG.role,
        PAGE_CONFIG.UPDATE_SERVICE_TAG.role,
      ],
      renderChildren: (item: any) => (
        <>
          <ActionEditButton
            role={PAGE_CONFIG.UPDATE_SERVICE_TAG.role}
            onClick={() => onUpdateButtonClick(item.id)}
          />
          <ActionDeleteButton
            role={PAGE_CONFIG.DELETE_SERVICE_TAG.role}
  ...
  const onDeleteButtonClick = (id: any) => {
    showDeleteDialog(
      configDeleteDialog({
        label: PAGE_CONFIG.DELETE_SERVICE_TAG.label,
  ...
  const onCreateButtonClick = () => {
    showCreateForm(
      configModalForm({
        label: PAGE_CONFIG.CREATE_SERVICE_TAG.label,
        fetchApi: tag.create,
  ...
  const onUpdateButtonClick = (id: any) => {
    showUpdateForm(
      configModalForm({
        label: PAGE_CONFIG.UPDATE_SERVICE_TAG.label,
        fetchApi: tag.update,
        refreshData: handleRefreshData,
        hideModal: hideUpdateForm,
        setToast,
  ...
  return (
    <Sidebar
      breadcrumbs={[
        {
          label: PAGE_CONFIG.SERVICE_TAG.label,
        },
      ]}
      activeItem={PAGE_CONFIG.SERVICE_TAG.name}
      renderContent={
        <>
          ...
          <ToolBar
            ...
            actionButtons={
              <CreateButton
                role={PAGE_CONFIG.CREATE_SERVICE_TAG.role}
