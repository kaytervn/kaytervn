...
const UpdateNotificationGroup = ({ isVisible, formConfig }: any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      resetForm();
      const res = await notificationGroup.get(formConfig.initForm.id);
      if (res.result) {
        const data = decryptData(
          sessionKey,
          res.data,
          DECRYPT_FIELDS.NOTIFICATION_GROUP
        );
        setForm({
          id: data.id,
          name: data.name,
...
export default UpdateNotificationGroup;
