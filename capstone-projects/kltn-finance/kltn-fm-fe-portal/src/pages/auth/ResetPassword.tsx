...
const ResetPassword = () => {
  ...
  useEffect(() => {
    if (
      !state?.userId ||
      !state?.grantType ||
      (GRANT_TYPE.EMPLOYEE === state.grantType && !state?.tenantId)
    ) {
  ...
  const handleSubmit = async () => {
    if (isValidForm()) {
      let res;
      if (GRANT_TYPE.CUSTOMER === state.grantType) {
        res = await auth.resetPassword({ userId: state.userId, ...form });
      } else {
        res = await auth.resetPasswordEmployee({
          userId: state.userId,
