...
const ViewTransactionPeriod = () => {
  const { id, transactionId } = useParams();
  const { sessionKey } = useGlobalContext();
  const { handleNavigateBack: backPayment } = useQueryState({
    path: PAGE_CONFIG.PAYMENT_PERIOD.path,
    requireSessionKey: true,
  });
  const { handleNavigateBack: backTranasction } = useQueryState({
    path: `/payment-period/view/${id}`,
    requireSessionKey: true,
  });
  const { transaction, loading } = useApi();
  const { tag, paymentPeriod, employee, category, transactionGroup } = useApi();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [pp, trs] = await Promise.all([
        paymentPeriod.get(id),
        transaction.get(transactionId),
      ]);
      if (!pp.result || !trs.result) {
        backPayment();
      }
      ...
      setFetchData(data);
      setPeriodData(
        decryptData(sessionKey, pp.data, DECRYPT_FIELDS.PAYMENT_PERIOD)
      );
      setForm({
    ...
    <Sidebar
      breadcrumbs={[
        ...
        },
        {
          ...
          onClick: backTranasction,
        },
        {
          label: PAGE_CONFIG.VIEW_TRANSACTION_PERIOD.label,
        },
      ]}
      ...
      renderContent={
        <>
          ...
          <FormCard
            title={PAGE_CONFIG.VIEW_TRANSACTION_PERIOD.label}
            children={
...
export default ViewTransactionPeriod;
