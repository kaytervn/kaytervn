...
parser.add_argument(
    "--framework",
    choices=["caffe", "tensorflow", "torch", "darknet", "dldt", "onnx"],
...
def postprocess(frame, outs):
    ...
    if (
        len(outNames) > 1
        or (lastLayer.type == "Region" or args.postprocessing == "yolov8")
        and args.backend != cv.dnn.DNN_BACKEND_OPENCV
    ):
        indices = []
...
cv.createTrackbar(
    "Confidence threshold, %", winName, int(confThreshold * 100), 99, callback
)
    ...
    while process:
        ...
        if not frame is None:
            ...
            blob = cv.dnn.blobFromImage(
                frame, size=(inpWidth, inpHeight), swapRB=args.rgb, ddepth=cv.CV_8U
            )
            ...
            if (
                net.getLayer(0).outputNameToIndex("im_info") != -1
