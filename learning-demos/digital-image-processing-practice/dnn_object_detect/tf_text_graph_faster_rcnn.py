...
def createFasterRCNNGraph(modelPath, configPath, outputPath):
    ...
    grid_anchor_generator = config["first_stage_anchor_generator"][0][
    ...
    if (
        "type" in feature_extractor
        and feature_extractor["type"][0] == "faster_rcnn_nas"
    ):
    ...
    def to_remove(name, op):
        ...
        return (
            op == "Const"
            or name.startswith(scopesToIgnore)
            or not name.startswith(scopesToKeep)
            or (name.startswith("CropAndResize") and op != "CropAndResize")
        )
    ...
    for node in graph_def.node:
        if (
            ...
            or node.name == "SecondStageBoxPredictor/Flatten_1/flatten/Reshape"
        ):
            node.op = "Flatten"
            node.input.pop()
    ...
    addReshape(
        "detection_out/slice", "detection_out/slice/reshape", [1, 1, -1], graph_def
    )
