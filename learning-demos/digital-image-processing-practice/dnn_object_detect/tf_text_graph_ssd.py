...
class MultiscaleAnchorGenerator:
    def __init__(self, min_level, aspect_ratios, scales_per_octave, anchor_scale):
        ...
        self.scales = [
            2 ** (float(s) / scales_per_octave) for s in range(scales_per_octave)
        ]
...
def createSSDGraph(modelPath, configPath, outputPath):
    ...
    box_predictor = (
        "convolutional"
        if "convolutional_box_predictor" in config["box_predictor"][0]
    ...
    if "ssd_anchor_generator" in anchor_generator:
        ...
        if "reduce_boxes_in_lowest_layer" in ssd_anchor_generator:
            reduce_boxes_in_lowest_layer = (
                ssd_anchor_generator["reduce_boxes_in_lowest_layer"][0] == "true"
            )
        priors_generator = SSDAnchorGenerator(
            min_scale,
            max_scale,
            num_layers,
            aspect_ratios,
        ...
        print(
            "Reduce boxes in the lowest layer: %s" % str(reduce_boxes_in_lowest_layer)
        )
        ...
        aspect_ratios = [
            float(ar) for ar in multiscale_anchor_generator["aspect_ratios"]
        ]
        scales_per_octave = int(multiscale_anchor_generator["scales_per_octave"][0])
        num_layers = max_level - min_level + 1
        priors_generator = MultiscaleAnchorGenerator(
            min_level, aspect_ratios, scales_per_octave, anchor_scale
        )
    ...
    def fuse_nodes(nodesToKeep):
        ...
        subgraphBatchNormV2 = [
            ...
            [
                "Mul",
                "input",
                ["Mul", ["Rsqrt", ["AddV2", "moving_variance", "add_y"]], "gamma"],
            ],
            ["Sub", "beta", ["Mul", "moving_mean", "Mul_0"]],
        ]
        ...
        subgraphResizeNN = [
            ...
            [
                ...
                [
                    "Reshape",
                    "input",
                    ["Pack", "shape_1", "shape_2", "shape_3", "shape_4", "shape_5"],
                ],
                "ones",
            ],
            [
                "Pack",
                ["StridedSlice", ["Shape", "input"], "stack", "stack_1", "stack_2"],
                "out_height",
                "out_width",
                "out_channels",
            ],
        ...
        for node in graph_def.node:
            ...
            fusedNodes = []
            if checkSubgraph(
                node, subgraphBatchNorm, inputs, fusedNodes
            ) or checkSubgraph(node, subgraphBatchNormV2, inputs, fusedNodes):
                name = node.name
                node.Clear()
                node.name = name
            ...
            if checkSubgraph(node, subgraphResizeNN, inputs, fusedNodes):
                ...
                out_height = int(
                    out_height_node.attr["value"]["tensor"][0]["int_val"][0]
                )
    ...
    if graph_def.node[2].name.startswith(preproc_id) and graph_def.node[2].input[
        0
    ].startswith(preproc_id):
    ...
    for label in [
        "ClassPredictor",
        "BoxEncodingPredictor" if box_predictor == "convolutional" else "BoxPredictor",
    ]:
        concatInputs = []
        for i in range(num_layers):
            # Flatten predictions
            ...
            if box_predictor == "convolutional":
                ...
                if i == 0:
                    ...
                    inpName = "WeightSharedConvolutionalBoxPredictor_%d/%s/BiasAdd" % (
                        i,
                        label,
                    )
    ...
    for node in graph_def.node:
        if (
            ...
            or re.match("BoxPredictor_\d/BoxEncodingPredictor/Conv2D", node.name)
            or re.match(
                "WeightSharedConvolutionalBoxPredictor(_\d)*/BoxPredictor/Conv2D",
                node.name,
            )
        ):
    ...
    boxCoderVariance = [
        1.0 / float(fasterRcnnBoxCoder["x_scale"][0]),
        1.0 / float(fasterRcnnBoxCoder["y_scale"][0]),
    ...
    for i in range(num_layers):
        ...
        if box_predictor == "convolutional":
            ...
            if i == 0:
                priorBox.input.append(
                    "WeightSharedConvolutionalBoxPredictor/BoxPredictor/Conv2D"
                )
            else:
                priorBox.input.append(
                    "WeightSharedConvolutionalBoxPredictor_%d/BoxPredictor/BiasAdd" % i
                )
    ...
    if "score_threshold" in batchNMS:
        detectionOut.addAttr(
            "confidence_threshold", float(batchNMS["score_threshold"][0])
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", required=True, help="Path to frozen TensorFlow graph."
    )
    parser.add_argument("--output", required=True, help="Path to output text graph.")
    parser.add_argument(
        "--config", required=True, help="Path to a *.config file is used for training."
    )
